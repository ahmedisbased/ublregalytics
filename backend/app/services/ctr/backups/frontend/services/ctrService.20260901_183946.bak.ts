...
export interface CsvBatchGeneratorParams {
  from_date: string;
  to_date: string;
  cr_dr_flag: "DR" | "CR";
  entity_individual_flag: "ENTITY" | "INDIVIDUAL";
...
export const uploadFileStream = async (form : FormData) => {
  ...
  const response = api.post("/ctr" , form, {
    responseType : 'blob',
    headers : {
...
export const uploadFilesStream = async (form: FormData) => {
  return api.post("/ctr/batch", form, {
    responseType: "blob",
    headers: {
      "Content-Type": "multipart/form-data",
    },
...
export const generateXml = async (formData: FormData) => {
  ...
  const response =  api.post("/ctr", formData, {
